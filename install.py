...
class CMD:
    ...
    INSTALL_SCADA="cd %s && echo -e \'#!/usr/bin/expect \nset timeout -1 \nspawn ./install_expect.sh > install.log  \nexpect \"*用户的密码\" {\nsend \"%s\\n\"\n}\ninteract\' > install.sh && chmod +x install.sh && chmod +x install_expect.sh && ./install.sh && rm -f install.sh"
    SET_SECURE="cd %s && echo -e \'#!/usr/bin/expect \nset timeout -1 \nspawn ./precautions.sh \nexpect \"*password\" {\nsend \"%s\\n\"\n}\n \nexpect \"*password\" {\nsend \"%s\\n\"\n}\ninteract\' > install2.sh && chmod +x install2.sh && chmod +x precautions.sh && ./install2.sh && rm -f install2.sh"
execute=CMD()
class Pysftp(object):
    """
    python3 自动上传、下载文件
